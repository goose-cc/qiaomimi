...
def compute_gy(x, fx, y):
    """
    Single-sample forward operator used by the existing MCTS code:

        g(y) = integral f(x) / (y - x) dx

    This is the single-vector counterpart of compute_gy_batch().
    """
    ...
    if len(x) != len(fx):
        raise ValueError(
            f"x and fx must have the same length, got {len(x)} and {len(fx)}."
        )

    denominator = y[:, None] - x[None, :]
    if np.any(np.isclose(denominator, 0.0)):
        raise ValueError("The forward kernel contains y == x, causing a singularity.")

    integrand = fx[None, :] / denominator

    if hasattr(np, "trapezoid"):
        gy = np.trapezoid(integrand, x=x, axis=1)
    else:
        gy = np.trapz(integrand, x=x, axis=1)

    return np.asarray(gy, dtype=np.float64)
