#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Train Exp23 narrow-peak weighted a1+gamma model.
...
This keeps the same PairwiseInverseMLP architecture and data representation as
the Exp23 baseline, but changes the training objective only:

1) log-gamma errors for small true gamma receive a moderate sample weight;
2) an optional peak-height proxy loss penalizes errors in log10(a1/gamma).

For fixed m, the resonance peak height scales approximately as a1/(m*gamma),
so log10(a1/gamma) is a cheap differentiable proxy for peak-shape fidelity.
No direct f(s) target is required in train/val files.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--narrow-focus-gamma", type=float, default=0.1)
    p.add_argument("--narrow-weight-strength", type=float, default=1.0)
    p.add_argument("--narrow-weight-power", type=float, default=0.5)
    p.add_argument("--narrow-weight-max", type=float, default=4.0)
    p.add_argument("--peak-proxy-loss-weight", type=float, default=0.25)
...
def narrow_sample_weights(
    ...
    *,
    ...
    """Moderately emphasize small-gamma samples, then normalize mean weight to 1."""
    gamma = torch.clamp(gamma, min=1e-12)
    ratio = torch.clamp(float(focus_gamma) / gamma, min=1.0)
    raw = torch.pow(ratio, float(power))
    weight = 1.0 + float(strength) * (raw - 1.0)
    weight = torch.clamp(weight, min=1.0, max=float(max_weight))
    return weight / torch.mean(weight).clamp_min(1e-12)
...
def exp23_loss(
    ...
    *,
    ...
    gamma_w,
    narrow_focus_gamma,
    narrow_weight_strength,
    narrow_weight_power,
    narrow_weight_max,
    peak_proxy_w,
):
    second_per = ((pred_second - second) / second_span) ** 2
    true_log_gamma = torch.log10(gamma)
    gamma_per = (pred_log_gamma - true_log_gamma) ** 2

    weights = narrow_sample_weights(
        gamma,
        focus_gamma=narrow_focus_gamma,
        strength=narrow_weight_strength,
        power=narrow_weight_power,
        max_weight=narrow_weight_max,
    )

    pred_gamma = torch.pow(10.0, pred_log_gamma)
    # a1 is strictly positive in this experiment.
    log_peak_pred = torch.log10(pred_second.clamp_min(1e-8) / pred_gamma.clamp_min(1e-8))
    log_peak_true = torch.log10(second.clamp_min(1e-8) / gamma.clamp_min(1e-8))
    peak_per = (log_peak_pred - log_peak_true) ** 2

    second_loss = torch.mean(second_per)
    gamma_loss = torch.mean(weights * gamma_per)
    peak_loss = torch.mean(weights * peak_per)
    total = float(second_w) * second_loss + float(gamma_w) * gamma_loss + float(peak_proxy_w) * peak_loss
    return total, second_loss.detach(), gamma_loss.detach(), peak_loss.detach()


def train_one_epoch(
    model, loader, optimizer, device, second_span, second_w, gamma_w,
    narrow_focus_gamma, narrow_weight_strength, narrow_weight_power,
    narrow_weight_max, peak_proxy_w,
):
    model.train()
    total = 0.0
    n = 0
    for batch in loader:
        ...
        loss, _, _, _ = exp23_loss(
            pred_second, pred_log_gamma, second, gamma,
            second_span=second_span,
            second_w=second_w,
            gamma_w=gamma_w,
            narrow_focus_gamma=narrow_focus_gamma,
            narrow_weight_strength=narrow_weight_strength,
            narrow_weight_power=narrow_weight_power,
            narrow_weight_max=narrow_weight_max,
            peak_proxy_w=peak_proxy_w,
        )
        loss.backward()
        optimizer.step()
...
@torch.no_grad()
def evaluate_loader(
    model, loader, device, second_span, second_w, gamma_w,
    narrow_focus_gamma, narrow_weight_strength, narrow_weight_power,
    narrow_weight_max, peak_proxy_w,
):
    model.eval()
...
def main() -> None:
    ...
    print("=" * 96)
    print(f"Exp23 narrow-peak weighted training: {args.mode}")
    ...
    print(f"narrow focus     : gamma <= {args.narrow_focus_gamma:g}")
    print(f"narrow weighting : strength={args.narrow_weight_strength:g}, power={args.narrow_weight_power:g}, max={args.narrow_weight_max:g}")
    print(f"peak proxy weight: {args.peak_proxy_loss_weight:g}")
    print("=" * 96)
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss = train_one_epoch(
            ...
            args.narrow_focus_gamma, args.narrow_weight_strength,
            args.narrow_weight_power, args.narrow_weight_max,
            args.peak_proxy_loss_weight,
        )
    ...
    run_meta = {
        "experiment": "Exp23 narrow-peak weighted training",
        "mode": args.mode,
        "second_parameter": second_name,
