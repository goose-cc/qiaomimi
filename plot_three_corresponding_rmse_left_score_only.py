...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output_dir", default="./picture/three_corresponding_rmse_left_score"
    )
...
def relative_rmse_percent(pred: np.ndarray, truth: np.ndarray) -> float:
    """NRMSE based on the RMS of the reference curve, expressed as %."""
    return 100.0 * rmse(pred, truth) / max(rms(truth), 1e-12)


def one_minus_nrmse_score(relative_rmse_pct: float) -> float:
    """Custom score: (1 - NRMSE) * 100%; this is not classification accuracy."""
    return 100.0 - relative_rmse_pct
...
def main() -> None:
    ...
    for order, (idx, pred) in enumerate(zip(indices, fx_pred), start=1):
        ...
        # 只对左侧 F 重建结果计算相对拟合度。
        # 不能直接用 1 - RMSE，因为 RMSE 有量纲；先用真实 F 的 RMS 归一化：
        # F_NRMSE = F_RMSE / RMS(F_true)，F_score = (1 - F_NRMSE) * 100%。
        # 右侧 G 图只用于展示 10% 加噪前后差异，因此仅报告 G_RMSE。
        relative_f_rmse = relative_rmse_percent(pred, true_f)
        f_score = one_minus_nrmse_score(relative_f_rmse)
        ...
        ax_f.set_title(
            ...
            f"F RMSE={f_rmse:.6f} | NRMSE={relative_f_rmse:.2f}% | "
            f"1-NRMSE score={f_score:.2f}%"
        )
        ax_f.legend()
        ...
        ax_left.set_title(
            ...
            f"RMSE={f_rmse:.6f}, NRMSE={relative_f_rmse:.2f}%, "
            f"score={f_score:.2f}%"
        )
        ...
        rows.append(
            {
                ...
                "f_rmse": f_rmse,
                "f_nrmse_percent": relative_f_rmse,
                "f_one_minus_nrmse_score_percent": f_score,
                "g_noise_rmse": g_rmse,
                "f_figure": str(f_path),
                "g_figure": str(g_path),
            }
        ...
        print(
            ...
            f"峰中心 m^2={peak_center:.3f}, "
            f"F_RMSE={f_rmse:.6f}, F_NRMSE={relative_f_rmse:.2f}%, "
            f"F_score={f_score:.2f}%; "
            f"G_RMSE={g_rmse:.6f}"
        )
    ...
    np.savez_compressed(
        ...
        f_nrmse_percent=np.asarray(
            [row["f_nrmse_percent"] for row in rows], dtype=np.float32
        ),
        f_one_minus_nrmse_score_percent=np.asarray(
            [row["f_one_minus_nrmse_score_percent"] for row in rows],
            dtype=np.float32,
        ),
    ...
    print("说明：只有左侧 F 重建图计算 NRMSE 和 score=(1-NRMSE)*100%。")
    print("右侧 G 加噪图仅报告噪声 RMSE，不计算预测精度。")
