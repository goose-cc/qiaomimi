...
Each montage uses one row per sample and three columns:
    total f(s) | resonance diagnostic | forward g(q^2)

All metrics are written inside the corresponding plot panel. There is no separate
...
def _add_panel_box(ax: Any, text: str, *, location: str = "lower right") -> None:
    positions = {
    ...
    ax.text(
        x, y, text, transform=ax.transAxes, ha=ha, va=va, fontsize=8.2,
        family="monospace",
        bbox=dict(boxstyle="round", facecolor="white", edgecolor="0.35", alpha=0.88),
        zorder=10,
    )
...
def plot_montage(
    *,
    ...
    fig_height = max(4.25 * n_rows, 7.8)
    fig, axes = plt.subplots(
        n_rows,
        3,
        figsize=(22.5, fig_height),
        squeeze=False,
    ...
    for row_pos, (i, candidate_rank) in enumerate(zip(selected, candidate_ranks)):
        ...
        # Column 1: total spectrum and total-spectrum metrics.
        ax_f = axes[row_pos, 0]
        ...
        ax_f.set_title(
            "Total spectrum | rank=%d | row=%d" % (candidate_rank, row_id),
            fontsize=10,
        )
        f_box = "\n".join([
            "MSE(scaled)  = %.6g" % record["f_mse_scaled"],
            ...
            "NRMSE/rel-L2 = %.4f" % record["f_relative_l2"],
            "recon score  = %.2f%%" % record["relative_reconstruction_score_percent"],
            "a1=%.4g  a2=%.4g  a3=%.4g" % (record["a1"], record["a2"], record["a3"]),
            "m=%.4g  gamma=%.4g" % (record["m"], record["gamma"]),
            "m*gamma=%.4g" % record["m_gamma"],
        ])
        _add_panel_box(ax_f, f_box, location="lower right")

        # Column 2: diagnostic resonance component and resonance metrics.
        ax_r = axes[row_pos, 1]
        ax_r.plot(
            s_grid, data["resonance_true"][i],
            label="true resonance", linewidth=2.0,
        )
        ax_r.plot(
            s_grid, data["resonance_pred_diag"][i],
            label="f_pred - true background", linewidth=2.0,
        )
        ax_r.axvline(
        ...
        ax_r.set_title("Resonance diagnostic", fontsize=10)
        resonance_box = "\n".join([
            "MSE(scaled)  = %.6g" % record["resonance_mse_scaled"],
            "RMSE(scaled) = %.6g" % record["resonance_rmse_scaled"],
            "rel-L2       = %.4f" % record["resonance_relative_l2"],
            "peak center Δ= %.4f" % record["peak_center_abs_error"],
            "peak height Δ= %.4f" % record["peak_height_relative_error"],
            "visibility   = %.4f" % record["resonance_visibility"],
            "width/grid   = %.2f" % record["width_grid_cells"],
        ])
        _add_panel_box(ax_r, resonance_box, location="lower right")

        # Column 3: forward g consistency and forward metrics.
        ax_g = axes[row_pos, 2]
        ...
        ax_g.plot(
            q2_grid, data["g_true_discrete"][i],
            label="K(f_true_100)", linewidth=1.2,
        )
        ax_g.set_xlabel("q^2")
        ax_g.set_ylabel("scaled g(q^2)")
        ax_g.grid(True, alpha=0.3)
        ax_g.legend(fontsize=7, loc="upper left")
        ax_g.set_title("Forward consistency", fontsize=10)
        g_box = "\n".join([
            "MSE vs clean  = %.6g" % record["g_mse_vs_clean_scaled"],
            "RMSE vs clean = %.6g" % record["g_rmse_vs_clean_scaled"],
            "g rel clean   = %.5f" % record["g_relative_l2_vs_clean"],
            "g rel discrete= %.5f" % record["g_relative_l2_vs_discrete_clean"],
            "repr. gap     = %.5f" % record["clean_discrete_representation_gap"],
        ])
        _add_panel_box(ax_g, g_box, location="lower right")
    ...
    fig.tight_layout(rect=(0.01, 0.02, 0.995, 0.982), h_pad=2.0, w_pad=1.5)
