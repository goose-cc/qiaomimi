#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Shared numerical core for Exp38 custom observation designs.
...
from mc_physics import legendre_rule
...
def forward64(
    ...
    if q2.size < 8 or not np.all(np.isfinite(q2)):
        raise ValueError("q2 design must contain at least 8 finite points")
...
def forward64_batched(parameters: np.ndarray, q2: np.ndarray, integration_points: int, batch_size: int = 768) -> np.ndarray:
    p = _as_params(parameters)
    parts = []
    for start in range(0, len(p), int(batch_size)):
        parts.append(forward64(p[start:start+int(batch_size)], q2=q2, integration_points=integration_points))
    if not parts:
        return np.empty((0, len(np.asarray(q2).reshape(-1))), dtype=np.float64)
...
def make_profile_bank(cfg: dict[str, Any], q2: np.ndarray, integration_points: int,
    ...
    pg = cfg.get("profile_grid", {})
    if m_points is None or log_gamma_points is None:
        if "m_points" not in pg or "log_gamma_points" not in pg:
            raise KeyError("profile grid sizes must be passed explicitly or provided as cfg[\"profile_grid\"]")
    M = int(m_points if m_points is not None else pg["m_points"])
    G = int(log_gamma_points if log_gamma_points is not None else pg["log_gamma_points"])
    ...
    background = forward64_batched(background_p, q2, integration_points)[0]
    ...
    g1 = forward64_batched(profile_p, q2, integration_points, batch_size=512)
...
def refine_alias_ids(ids, params, clean_g64, cfg, background, m_grid, lg_grid,
                     resonance, r2, grid_diag, q2, integration_points,
    ...
    def direct_rr(mv, lgv):
        ...
        return forward64(pp, q2=q2, integration_points=integration_points)[0] - background
...
def alias_margin_direct(true_p: np.ndarray, alias_p: np.ndarray, cfg: dict[str, Any], q2: np.ndarray, integration_points: int) -> float:
    gg = forward64(np.vstack([true_p, alias_p]), q2=q2, integration_points=integration_points)
...
def varpro_predict_one(observation: np.ndarray, cfg: dict[str, Any], background: np.ndarray,
                       ...
                       q2: np.ndarray, integration_points: int, seed_count: int = 3, maxiter: int = 60):
    ...
    for i, j, idx in seeds:
        ...
        mv, lgv = float(opt.x[0]), float(opt.x[1])
        pp = np.array([[1.0, fixed["a2"], fixed["a3"], mv, 10.0**lgv]], dtype=np.float64)
        rr = forward64(pp, q2=q2, integration_points=integration_points)[0] - background
    ...
    fitted = background + aa * (
        forward64(
            ...
            q2=q2, integration_points=integration_points,
        )[0] - background
    )
