#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Exp19A: gamma-only curriculum regression.

Scientific question
-------------------
With a1, a2, a3 and m fixed and the gamma values deliberately separated in
g-space, can a simple network learn g(q^2) -> gamma?

This script:
1. trains ONLY on data_gamma_easy/noise_0p2pct by default;
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Exp19A gamma-only Easy regression",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--data-dir", default="./data_gamma_easy")
    parser.add_argument("--train-noise-dir", default="noise_0p2pct")
    parser.add_argument("--output-dir", default="./validation_results/exp19_gamma_only")
...
def main() -> None:
    ...
    checkpoint_path = output_dir / "best_gamma_only_mlp.pt"
    ...
    print("=" * 96)
    print("Exp19A: gamma-only Easy regression")
    ...
    write_csv(output_dir / "exp19_summary.csv", summary_rows)
    write_csv(output_dir / "exp19_samples.csv", sample_rows)
    plot_predictions(sample_rows, output_dir, "test_seen")
    plot_predictions(sample_rows, output_dir, "test_interp")

    metadata = {
        "experiment": "Exp19A gamma-only Easy regression",
        "scientific_question": (
            "Can a simple MLP recover gamma when a1,a2,a3,m are fixed and "
            "the gamma states are deliberately separated in g-space?"
        ),
    ...
    with (output_dir / "exp19_metadata.json").open("w", encoding="utf-8") as handle:
        json.dump(metadata, handle, ensure_ascii=False, indent=2)

    print("=" * 96)
    print("Exp19A finished.")
    print(f"output: {output_dir}")
    print("Read first:")
    print(f"  {output_dir / 'exp19_summary.csv'}")
    print(f"  {output_dir / 'exp19_samples.csv'}")
    print("=" * 96)
